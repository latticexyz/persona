...
contract Persona is ERC721 {
    ...
    function gradientForAddress(address user) public view returns (bytes[4] memory) {
        uint256 personaId = getPersona(user);
        return LibCustomArt.gradientForAddress(personaId);
    }

    function barsForAddress(address user) public view returns (uint8[32] memory) {
        uint256 personaId = getPersona(user);
        return LibCustomArt.barsForAddress(personaId);
    }
